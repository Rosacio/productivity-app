...
function enrichEvents(events, eventsAreSorted) {
    ...
    var eventsWithOverlaps = baseEvents.map(function (event, index) {
        ...
        return tslib.__assign(tslib.__assign({}, event), { 
    ...
    eventsWithOverlaps.forEach(function (event, index) {
        ...
        var enrichedEvent = tslib.__assign(tslib.__assign({}, event), { overlapCount: overlapCountingPointers[index] });
        var startDate = dayjs__default["default"](enrichedEvent.start).format(SIMPLE_DATE_FORMAT);
        var endDate = dayjs__default["default"](enrichedEvent.end).format(SIMPLE_DATE_FORMAT);
        ...
        if (startDate === endDate) {
            ...
            eventsByDate[startDate].push(tslib.__assign(tslib.__assign({}, enrichedEvent), { end: dayjs__default["default"](enrichedEvent.start).endOf('day').toDate() }));
            // Add events in the bucket of the middle dates
            var amountOfDaysBetweenDates = dayjs__default["default"](enrichedEvent.start).diff(enrichedEvent.end, 'day');
            for (var i = 1; i <= amountOfDaysBetweenDates; i++) {
                var intermediateDate = dayjs__default["default"](enrichedEvent.start).add(1, 'day');
                if (!eventsByDate[intermediateDate.format(SIMPLE_DATE_FORMAT)]) {
                    eventsByDate[intermediateDate.format(SIMPLE_DATE_FORMAT)] = [];
                }
                eventsByDate[intermediateDate.format(SIMPLE_DATE_FORMAT)].push(tslib.__assign(tslib.__assign({}, enrichedEvent), { start: intermediateDate.startOf('day').toDate() }));
            }
            // Add the event to the bucket of the end date
            eventsByDate[endDate].push(tslib.__assign(tslib.__assign({}, enrichedEvent), { start: dayjs__default["default"](enrichedEvent.end).startOf('day').toDate() }));
        }
    });
    return eventsByDate;
}
...
function getEventSpanningInfo(event, date, dayOfTheWeek, calendarWidth, showAdjacentMonths) {
    ...
    var eventDuration = Math.floor(dayjs__default["default"].duration(dayjs__default["default"](event.end).endOf('day').diff(dayjs__default["default"](event.start))).asDays()) + 1;
    var eventDaysLeft = Math.floor(dayjs__default["default"].duration(dayjs__default["default"](event.end).endOf('day').diff(date)).asDays()) + 1;
...
function usePanResponder(_a) {
    ...
    var panHandledRef = React__default["default"].useRef(false);
...
function useCalendarTouchableOpacityProps(_a) {
    ...
    var plainJsEvent = React__default["default"].useMemo(function () { return (tslib.__assign(tslib.__assign({}, event), { start: dayjs__default["default"](event.start).toDate(), end: dayjs__default["default"](event.end).toDate() })); }, [event]);
    var _onPress = React__default["default"].useCallback(function () {
        onPressEvent && onPressEvent(plainJsEvent);
    }, [onPressEvent, plainJsEvent]);
    var touchableOpacityProps = tslib.__assign({ delayPressIn: 20, key: "".concat(event.start.toISOString(), "_").concat(event.title), style: tslib.__spreadArray(tslib.__spreadArray([eventCellCss.style], injectedStyles, true), [getEventStyle(plainJsEvent)], false), onPress: _onPress, disabled: !onPressEvent || !!event.disabled }, eventCellAccessiblityProps);
...
function DefaultCalendarEventRenderer(_a) {
    ...
    return (React__namespace.createElement(reactNative.TouchableOpacity, tslib.__assign({}, touchableOpacityProps), dayjs__default["default"](event.end).diff(event.start, 'minute') < 32 && showTime ? (React__namespace.createElement(reactNative.Text, { style: eventTitleStyle },
        event.title,
        ",",
...
var getEventCellPositionStyle = function (start, end, minHour, hours) {
    var totalMinutesInRange = (DAY_MINUTES / 24) * hours;
    var durationInMinutes = dayjs__default["default"](end).diff(start, 'minute');
    var relativeHeight = 100 * (1 / totalMinutesInRange) * durationInMinutes;
    var relativeTop = getRelativeTopInDay(dayjs__default["default"](start), minHour, hours);
...
function _CalendarEvent(_a) {
    ...
    var theme = useTheme();
    var palettes = React__namespace.useMemo(function () { return tslib.__spreadArray([theme.palette.primary], theme.eventCellOverlappings, true); }, [theme]);
    ...
    return (React__namespace.createElement(DefaultCalendarEventRenderer, { event: event, showTime: showTime, ampm: ampm, touchableOpacityProps: touchableOpacityProps, textColor: eventCellTextColor || textColor }));
}
...
var _HourGuideCell = function (_a) {
    ...
    return (React__namespace.createElement(reactNative.TouchableWithoutFeedback, tslib.__assign({ onLongPress: function () { return onLongPress(date.hour(hour).minute(0)); }, onPress: function () { return onPress(date.hour(hour).minute(0)); } }, calendarCellAccessibilityProps),
        React__namespace.createElement(reactNative.View, { style: [
            ...
            ] }, Array.from({ length: timeslots }, function (_, index) { return (React__namespace.createElement(reactNative.View, { key: index, style: [
                u['border-l'],
                u['border-b'],
                {
...
var _HourGuideColumn = function (_a) {
    ...
    return (React__namespace.createElement(reactNative.View, tslib.__assign({ style: { height: cellHeight } }, calendarCellAccessibilityProps), HourComponent ? (React__namespace.createElement(HourComponent, { hour: hour, ampm: ampm })) : (React__namespace.createElement(reactNative.Text, { style: [objHasContent(hourStyle) ? hourStyle : textStyle, u['text-center']] }, formatHour(hour, ampm)))));
};
...
function _CalendarBody(_a) {
    ...
    var _onLongPressCell = React__namespace.useCallback(function (date) {
        onLongPressCell && onLongPressCell(date.toDate());
    }, [onLongPressCell]);
    var internalEnrichedEventsByDate = React__namespace.useMemo(function () {
    ...
    var enrichedEvents = React__namespace.useMemo(function () {
        if (enableEnrichedEvents)
            return [];
        if (isEventOrderingEnabled) {
            ...
            return sortedEvents_1.map(function (event) { return (tslib.__assign(tslib.__assign({}, event), { overlapPosition: getOrderOfEvent(event, sortedEvents_1), overlapCount: getCountOfEventsAtEvent(event, sortedEvents_1) })); });
        }
        return events;
    ...
    var _renderEvents = React__namespace.useCallback(function (date) {
        if (enableEnrichedEvents) {
            ...
            return (React__namespace.createElement(React__namespace.Fragment, null,
                ...
                enrichedEvents
                    ...
                    .map(function (event) { return (tslib.__assign(tslib.__assign({}, event), { start: dayjs__default["default"](event.end).startOf('day') })); })
                    .map(_renderMappedEvent),
                enrichedEvents
                    .filter(function (_a) {
                    var start = _a.start, end = _a.end;
                    ...
                    .map(function (event) { return (tslib.__assign(tslib.__assign({}, event), { start: dayjs__default["default"](event.end).startOf('day'), end: dayjs__default["default"](event.end).endOf('day') })); })
                    .map(_renderMappedEvent)));
        }
    ...
    return (React__namespace.createElement(React__namespace.Fragment, null,
        ...
        React__namespace.createElement(reactNative.ScrollView, tslib.__assign({ style: [
                {
                    height: containerHeight - cellHeight * 3,
                },
                style,
            ], ref: scrollView, scrollEventThrottle: 32 }, (reactNative.Platform.OS !== 'web' ? panResponder.panHandlers : {}), { showsVerticalScrollIndicator: showVerticalScrollIndicator, scrollEnabled: scrollEnabled, nestedScrollEnabled: true, contentOffset: reactNative.Platform.OS === 'ios' ? { x: 0, y: scrollOffsetMinutes } : { x: 0, y: 0 } }),
            React__namespace.createElement(reactNative.View, tslib.__assign({ style: [u['flex-1'], theme.isRTL ? u['flex-row-reverse'] : u['flex-row']] }, (reactNative.Platform.OS === 'web' ? panResponder.panHandlers : {})),
                (!hideHours || showWeekNumber) && (React__namespace.createElement(reactNative.View, { style: [u['z-20'], u['w-50']] }, hours.map(function (hour) { return (React__namespace.createElement(HourGuideColumn, { key: hour, cellHeight: cellHeight, hour: hour, ampm: ampm, hourStyle: hourStyle, calendarCellAccessibilityProps: calendarCellAccessibilityProps, hourComponent: hourComponent })); }))),
...
function _CalendarEventForMonthView(_a) {
    ...
    return (React__namespace.createElement(reactNative.TouchableOpacity, { style: [{ minHeight: eventMinHeightForMonthView }, u['mt-2']], onPress: function () { return !event.disabled && (onPressEvent === null || onPressEvent === void 0 ? void 0 : onPressEvent(event)); } }, (!isMultipleDays && date.isSame(event.start, 'day')) ||
        (isMultipleDays && isMultipleDaysStart) ? (renderEvent ? (renderEvent(event, touchableOpacityProps)) : (React__namespace.createElement(reactNative.View, tslib.__assign({}, touchableOpacityProps),
...
function _CalendarBodyForMonthView(_a) {
    ...
    var getStartOfWeek = React__namespace.useCallback(function (date) {
    ...
    var sortedEvents = React__namespace.useCallback(function (day) {
        if (!sortedMonthView) {
             ...
             */
            var startOfWeek_1 = getStartOfWeek(day);
            //filter all events that starts from the current week until the current day, and sort them by reverse starting time
            var filteredEvents = events
                .filter(function (_a) {
                var start = _a.start, end = _a.end;
                return dayjs__default["default"](end).isAfter(startOfWeek_1) && dayjs__default["default"](start).isBefore(max_1);
            })
                .sort(function (a, b) {
             ...
             */
            filteredEvents.forEach(function (_a) {
                var start = _a.start, end = _a.end;
            ...
            while (!tmpDay_1.isAfter(day)) {
                if (tmpDay_1 === startOfWeek_1) {
                    finalEvents_1 = tslib.__spreadArray([], filteredEvents, true);
                }
                finalEvents_1.forEach(function (event) {
                    if (dayjs__default["default"](event.end).isBefore(tmpDay_1.startOf('day')) ||
                        dayjs__default["default"](event.end).isSame(tmpDay_1.startOf('day'))) {
                        var eventsToMoveUp = finalEvents_1.filter(function (e) {
                            return dayjs__default["default"](e.start).startOf('day').isSame(tmpDay_1.startOf('day'));
                        });
                        if (eventsToMoveUp.length > 0) {
                            eventsToMoveUp.forEach(function (eventToMoveUp) {
                                var eventIndex = finalEvents_1.indexOf(event);
                                var eventToMoveUpIndex = finalEvents_1.indexOf(eventToMoveUp);
                                if (eventIndex > -1 && eventIndex < eventToMoveUpIndex) {
                                    finalEvents_1.splice(eventToMoveUpIndex, 1);
            ...
            finalEvents_1 = finalEvents_1.filter(function (e) {
                return !(dayjs__default["default"](e.end).endOf('day').isSame(dayjs__default["default"](day).startOf('day')) ||
                    dayjs__default["default"](e.end).endOf('day').isBefore(dayjs__default["default"](day).startOf('day')));
            });
    ...
    var renderDateCell = function (date, index) {
        ...
        return (React__namespace.createElement(reactNative.Text, { style: [
                { textAlign: 'center' },
                theme.typography.sm,
                {
    ...
    return (React__namespace.createElement(reactNative.View, tslib.__assign({ style: [
            {
            ...
            reactNative.Platform.OS === 'android' && style, // TODO: in Android, backgroundColor is not applied to child components
            {
                minHeight: minCellHeight,
            },
        ] },
        showWeekNumber ? (React__namespace.createElement(reactNative.View, tslib.__assign({ style: [
                i > 0 && u['border-t'],
                { borderColor: theme.palette.gray['200'] },
                u['p-2'],
                u['w-20'],
        ...
        week
            ...
            .map(function (date, ii) { return (React__namespace.createElement(reactNative.TouchableOpacity, tslib.__assign({ onLongPress: function () { return date && onLongPressCell && onLongPressCell(date.toDate()); }, onPress: function () { return date && onPressCell && onPressCell(date.toDate()); }, style: [
                i > 0 && u['border-t'],
                ...
                tslib.__assign({}, getCalendarCellStyle(date === null || date === void 0 ? void 0 : date.toDate(), i)),
            ...
            React__namespace.createElement(reactNative.TouchableOpacity, tslib.__assign({ onPress: function () {
                    return date &&
                        (onPressDateHeader
            ...
            calendarWidth > 0 &&
                ...
                date &&
                sortedEvents(date).reduce(function (elements, event, index, events) { return tslib.__spreadArray(tslib.__spreadArray([], elements, true), [
                    index > maxVisibleEventCount ? null : index === maxVisibleEventCount ? (React__namespace.createElement(reactNative.Text, { key: index, style: [
                            theme.typography.moreLabel,
                            { marginTop: 2, color: theme.palette.moreLabel },
                        ], onPress: function () { return onPressMoreLabel === null || onPressMoreLabel === void 0 ? void 0 : onPressMoreLabel(events, date.toDate()); } }, moreLabel.replace('{moreCount}', "".concat(events.length - maxVisibleEventCount)))) : (React__namespace.createElement(CalendarEventForMonthView, { key: index, event: event, eventCellStyle: eventCellStyle, eventCellAccessibilityProps: eventCellAccessibilityProps, onPressEvent: onPressEvent, renderEvent: renderEvent, date: date, dayOfTheWeek: ii, calendarWidth: calendarWidth, isRTL: theme.isRTL, eventMinHeightForMonthView: eventMinHeightForMonthView, showAdjacentMonths: showAdjacentMonths })),
                ], false); }, []),
            disableMonthEventCellPress &&
            ...
            React__namespace.createElement(TouchableGradually, tslib.__assign({ style: {
...
function TouchableGradually(_a) {
    ...
    var backgroundColor = React__namespace.useRef(new reactNative.Animated.Value(0)).current;
    var handlePressIn = function () {
        reactNative.Animated.timing(backgroundColor, {
            toValue: 1,
            duration: 200,
    ...
    return (React__namespace.createElement(reactNative.TouchableHighlight, { onLongPress: onLongPress, onPressIn: handlePressIn, onPressOut: handlePressOut, onPress: onPress, underlayColor: "transparent", style: style },
        React__namespace.createElement(reactNative.Animated.View, { style: [
                {
...
function _CalendarHeader(_a) {
    ...
    return (React__namespace.createElement(reactNative.View, tslib.__assign({ style: [
        ...
        (!hideHours || showWeekNumber) && (React__namespace.createElement(reactNative.View, { style: [u['z-10'], u['w-50'], u['pt-2'], borderColor] }, showWeekNumber ? (React__namespace.createElement(reactNative.View, tslib.__assign({ style: [
                { height: cellHeight },
            ...
            React__namespace.createElement(reactNative.View, { style: objHasContent(dayHeaderStyle) ? dayHeaderStyle : [u['mb-6']] },
                React__namespace.createElement(reactNative.Text, { style: [
                        {
                            color: theme.palette.gray['800'],
                        },
        ...
        dateRange.map(function (date) {
            ...
            return (React__namespace.createElement(reactNative.TouchableOpacity, tslib.__assign({ style: [u['flex-1'], u['pt-2']], onPress: function () { return _onPressHeader(date.toDate()); }, disabled: onPressDateHeader === undefined, key: date.toString() }, headerCellAccessibilityProps),
                React__namespace.createElement(reactNative.View, { style: [
                        { height: cellHeight },
                        objHasContent(headerContentStyle) ? headerContentStyle : u['justify-between'],
                    ] },
                ...
                showAllDayEventCell ? (React__namespace.createElement(reactNative.View, { style: [
                    ...
                    return (React__namespace.createElement(reactNative.TouchableOpacity, tslib.__assign({ style: [eventCellCss.style, primaryBg, u['mt-2'], getEventStyle(event)], key: index, onPress: function () { return _onPressEvent(event); } }, allDayEventCellAccessibilityProps),
                        React__namespace.createElement(reactNative.Text, { style: {
...
function _CalendarHeaderForMonthView(_a) {
    ...
    return (React__namespace.createElement(reactNative.View, tslib.__assign({ style: [
        ...
        showWeekNumber ? (React__namespace.createElement(reactNative.View, tslib.__assign({ style: [u['w-20'], { paddingTop: 2 }], key: 'weekNumber' }, headerCellAccessibilityProps),
            React__namespace.createElement(reactNative.View, { style: { flex: 1, height: 30 } },
                React__namespace.createElement(reactNative.Text, { style: [
                        u['text-center'],
                        {
                            color: theme.palette.gray['800'],
        ...
        dates.map(function (date) { return (React__namespace.createElement(reactNative.View, tslib.__assign({ style: { flex: 1, paddingTop: 2 }, key: date.toISOString() }, headerCellAccessibilityProps),
...
function _Schedule(_a) {
     ...
     */
    var eventStyles = React__default["default"].useCallback(function (event) {
        // This default style  need for Schedule view
        var defaultEventStyle = tslib.__assign(tslib.__assign({}, u['flex-column']), u['h-50']);
        if (Array.isArray(eventCellStyle)) {
            return tslib.__spreadArray(tslib.__spreadArray([], [defaultEventStyle], false), eventCellStyle, true);
        }
        if (typeof eventCellStyle === 'object') {
            return tslib.__assign(tslib.__assign({}, defaultEventStyle), eventCellStyle);
        }
        if (typeof eventCellStyle === 'function') {
            var output = eventCellStyle(event);
            if (Array.isArray(output)) {
                return tslib.__spreadArray(tslib.__spreadArray([], [defaultEventStyle], false), output, true);
            }
            if (typeof output === 'object') {
                return tslib.__assign(tslib.__assign({}, defaultEventStyle), output);
            }
        }
     ...
     */
    var getItem = React__default["default"].useMemo(function () {
        var groupedData = events
            .sort(function (a, b) { return dayjs__default["default"](a.start).diff(b.start); })
            .reduce(function (result, item) {
            var startDate = dayjs__default["default"](item.start).format(SIMPLE_DATE_FORMAT);
            if (!result[startDate]) {
                result[startDate] = [];
            }
    ...
    var renderMonthSeparator = function (date) { return (React__default["default"].createElement(reactNative.View, { style: { width: '100%' } },
        React__default["default"].createElement(reactNative.Text, { style: [
    ...
    var renderFlatListItem = function (eventGroup, index) {
        var date = dayjs__default["default"](eventGroup[0].start).locale(locale);
        var shouldHighlight = activeDate ? date.isSame(activeDate, 'date') : isToday(date);
        var isNewMonth = index === 0 || !dayjs__default["default"](eventGroup[0].start).isSame(getItem[index - 1][0].start, 'month');
        return (React__default["default"].createElement(reactNative.View, { style: [u['flex'], { padding: 2, flexWrap: 'wrap' }] },
            isNewMonth && renderMonthSeparator(date),
            React__default["default"].createElement(reactNative.View, { style: [u['flex'], u['justify-center'], { width: '20%' }] },
                React__default["default"].createElement(reactNative.View, tslib.__assign({ style: [
            ...
            React__default["default"].createElement(reactNative.View, { style: [u['flex'], u['flex-column'], { width: '75%' }] }, eventGroup.map(function (event, index) {
                return (React__default["default"].createElement(reactNative.View, { style: [u['flex-1'], u['overflow-hidden'], { marginTop: 2, marginBottom: 2 }], key: index },
    ...
    return (React__default["default"].createElement(reactNative.View, { style: tslib.__assign(tslib.__assign({}, style), { height: containerHeight }) },
        React__default["default"].createElement(reactNative.FlatList, { data: getItem, renderItem: function (_a) {
                var item = _a.item;
...
function _CalendarContainer(_a) {
    ...
    var _25 = React__default["default"].useState(function () { return dayjs__default["default"](date); }), targetDate = _25[0], setTargetDate = _25[1];
    React__default["default"].useEffect(function () {
        if (dateString) {
            setTargetDate(dayjs__default["default"](dateString));
        }
    ...
    var getDateRange = React__default["default"].useCallback(function (date) {
        switch (mode) {
            case 'month':
...
function _Calendar(_a) {
    var _b = _a.theme, theme = _b === void 0 ? defaultTheme : _b, isRTL = _a.isRTL, props = tslib.__rest(_a, ["theme", "isRTL"]);
    ...
    return (React__default["default"].createElement(ThemeContext.Provider, { value: _theme },
        React__default["default"].createElement(CalendarContainer, tslib.__assign({}, props))));
}
...
exports.SIMPLE_DATE_FORMAT = SIMPLE_DATE_FORMAT;
